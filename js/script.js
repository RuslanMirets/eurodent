...
$('.video').click(function () { 
    $('.video').not(this).find('video').each(function(){
...
$('.select__list label').click(function () {
    ...
    $(this).closest('.select').find('.select__main span[data-target=' + selectcurrent + ']').addClass("active");
});
...
$('.doctor__education .ref_dashed').click(function () {
  ...
  $(this).find('span').toggleClass("active");
    $(this).closest('.doctor__education').find('.doctor__education-items_more').toggleClass("active");
    if ($(window).scrollTop() > pos_absolute_02 - 25) {
    // Если страницу прокрутили дальше, чем высота родителя минус высота фикс. блока, то стопорим блок
            $('.doctor__card').css({
                'position': 'absolute',
